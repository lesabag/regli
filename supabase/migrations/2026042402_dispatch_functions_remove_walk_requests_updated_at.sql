...
BEGIN
  ...
  IF v_next_walker_id IS NULL THEN
    UPDATE public.walk_requests
    SET smart_dispatch_state = 'idle',
        smart_dispatch_last_error = 'Missing dispatch candidate for next rank'
    WHERE id = p_request_id;
  ...
  PERFORM public.log_dispatch_event(
    ...
    jsonb_build_object(
      'attempt_no', v_next_rank,
      'rank', v_next_rank,
      'walker_id', v_next_walker_id,
      'dispatch_version', 1,
...
BEGIN
  ...
  WHERE request_id = p_request_id
    ...
    AND rank = COALESCE(v_attempt.rank, v_attempt.attempt_no);
